...
value_model = GPT2HeadWithValueModel.from_pretrained(config["vf_model_name"]).to(other_device)
...
def evaluate(eval_batch):
    ...
    query_tensors = [torch.tensor(t).long().to(other_device) for t in eval_batch["tokens"]]
    ...
    for i in range(len(query_tensors)):
        ...
        output_ref = model_ref.generate(
            query_tensors[i].unsqueeze(dim=0).to(other_device),
        ...
        output = model.generate(
            query_tensors[i].unsqueeze(dim=0).to(model_device),
            max_length=query_len + config["output_size"],
            **gen_kwargs,
        ).squeeze()
...
for epoch in range(total_epochs):
    ...
    for step, batch in tqdm(zip(range(total_ppo_steps), iter(dataloader))):
        ...
        t0 = time.time()
        query_tensors = [torch.tensor(t).long().to(model_device) for t in batch["tokens"]]
